...
export default function PackingList() {
  ...
  const [packingList, setPackingList] = useState<{
    [category: string]: { name: string; checked: boolean }[];
  }>({});
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          Object.entries(data).forEach(([category, items]) => {
            if (Array.isArray(items)) {
              if (typeof items[0] === "string") {
                // Old format
                migratedList[category] = items.map((item) => ({
                  name: item,
                  checked: false,
                }));
              } else {
                // New format
                migratedList[category] = items as {
                  name: string;
                  checked: boolean;
                }[];
              }
          ...
          Object.entries(geminiResponse).forEach(([category, items]) => {
            if (Array.isArray(items)) {
              validatedList[category] = items.map((item) => ({
                name: item,
                checked: false,
              }));
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollContainer}>
        ...
        <Text style={styles.title}>
          {getSubstringBeforeComma(destination)} Packing List
        </Text>
...
const styles = StyleSheet.create({
  ...
  scrollContainer: { flex: 1, padding: 20, backgroundColor: "#fff" },
  title: {
    fontSize: 24,
    fontWeight: "bold",
    marginBottom: 20,
